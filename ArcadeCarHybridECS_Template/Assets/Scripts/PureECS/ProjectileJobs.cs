...
public class ProjectileJobs : MonoBehaviour
{

    private NativeArray<Vector3> velocities, tempVelocities;
    private NativeArray<Vector3> positions, tempPositions;
    private NativeArray<Matrix4x4> renderMatrices, tempRenderMatrices;
    private Matrix4x4[] renderMatrixArray, tempRenderMatrixArray;
    private NativeArray<int> sleepingTimer, tempSleepingTimer;
    private NativeQueue<int> asleep, tempAsleep;
    ...
    IEnumerator Begin()
    {
        ...
        spawnDirection = GameObject.FindGameObjectWithTag("Player").transform;
    ...
    private void Respawn(int i)
    {
        ...
        velocities[i] = (Random.onUnitSphere + (carTransform.forward + carTransform.up/5) * 100f).normalized * Random.Range(5f, 10f); // spawn with velocities arching towards a target object
        positions[i] = carTransform.position;
        sleepingTimer[i] = 0;
    ...
    private void Init(int i)
    {
        ...
        //velocities[i] = Random.onUnitSphere * Random.Range(2, 10); // spawn objects with velocities spreading them out in a sphere
        //velocities[i] = (Random.onUnitSphere + (((spawnDirection.position - transform.position).normalized).normalized) * 1.3f).normalized * Random.Range(2f, 10f); // spawn with velocities arching towards a target object
        velocities[i] = (Random.onUnitSphere / 10 + carTransform.forward * .1f).normalized * Random.Range(10f, 20f); // spawn with velocities arching towards a target object
        positions[i] = carTransform.position;
        sleepingTimer[i] = 999;
        renderMatrices[i] = Matrix4x4.identity;
    }
